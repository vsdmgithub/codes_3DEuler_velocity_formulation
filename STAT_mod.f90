module statistics
    ...
    integer(kind=4)::i_x,i_y,i_z,slot
    double precision::bin_size,data_temp
    bin_size=(data_max-data_min)/DBLE(pdf_points)
    pdf=0.0D0
    pdf_w=0.0D0
    pdf_b(0)=data_min
    ...
    DO i_y=0,N-1
    DO i_z=0,N-1
        data_temp=data_sample(i_x,i_y,i_z)
        if (data_temp .NE. data_temp) then
            data_temp=0.0D0
        end if
        if (data_temp .LT. data_min) then
            slot=0
        elseif  (data_temp .GT. data_max) then
            slot=pdf_points
        else
            slot=FLOOR((data_temp-data_min)/bin_size)
        end if
        pdf(slot)=pdf(slot)+1.0D0

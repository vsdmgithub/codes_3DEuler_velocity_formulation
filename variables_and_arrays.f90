...
! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	INTEGER (KIND=4)::N,Nh,k_G,k_G_2,k_max
    ...
    INTEGER (KIND=4)::viscosity_status,purging_status,pvd_status,vortex_calc_status 
    ! _________________________
    ...
    ! _________________________
    ! VORTEX STRETCHING CALC VARIABLES
    ! !!!!!!!!!!!!!!!!!!!!!!!!!
    INTEGER(KIND=4)::bin,pdf_points,quaternion_tag
    CHARACTER(LEN=60)::pdf_chr
    ...
    ! _________________________________________
    ! VORTEX STRETCHING CALC. ARRAYS
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE::quaternion,w_dot,w_dot_parallel
    DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::pdf_data,pdf_data_w,pdf_bins
    DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE::w_mod             ! modulus of vorticity      
    DOUBLE PRECISION,DIMENSION(:,:,:),ALLOCATABLE::str_xx,str_yy,str_zz,str_xy,str_yz,str_zx ! StraIN tensor IN real space
    ...
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
        ...
        READ(10,f_d8p4,ADVANCE='yes')
        READ(10,f_i4,ADVANCE='yes'),vortex_calc_status ! Vortex stretchINg calc. ON or OFF
        READ(10,f_d8p4,ADVANCE='yes')
        ...
        norm_factor=DSQRT(8.0D0) ! For TG vortex. Otherwise normalize to 1 before time forward march.
        ...
        file_dir='data_'//TRIM(ADJUSTL(N_char))//'_TG/'
        file_dir_pvd='data_'//TRIM(ADJUSTL(N_char))//'_TG_PVD/'
        ...
        IF (vortex_calc_status .EQ. 1) THEN
            
            pdf_points=200
            ! No of bins in the histogram, which is normalized to get pdf.
...
!            file_name_pvd_vel='velocity_file'
!            file_name_pvd_vor='vorticity_file'
!            file_name_pvd_str='strain_component_file'
!            file_name_pvd_str_bg='bg_strain_component_file'
        END IF
...
!            CALL IC_KP
    ...
    ! ------------
    ! CALL THIS SUBROUTINE TO:
    !   This declares the purging matrix. 
    ! -------------
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    IMPLICIT  NONE
! -----------------------------------------------------------------
...
! -----------------------------------------------------------------------
    ...
    ! ------------
    ! CALL THIS SUBROUTINE TO:
    ! Initialize single mode initial condition called 'Taylor-Green'
    ! Vortex. Modes are (\pm 1,\pm 1,\pm 1) upto inversions.
    ! -------------
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  
    IMPLICIT  NONE
    DOUBLE COMPLEX::v1
    ...
    ! --------------------------------------------------------------------------------------------
     ...
     OPEN(unit=23,file=TRIM(ADJUSTL(file_dir))//TRIM(ADJUSTL(file_name_param))//'.dat')
        ...
        WRITE(23,"(A80)")'------------------------------------------'
        WRITE(23,"(A50,I4)")TRIM(ADJUSTL('N                  = ')),N
        WRITE(23,"(A50,I3)")TRIM(ADJUSTL('k_Galerkin.Trunc   = ')),k_G
        WRITE(23,"(A50,I2)")TRIM(ADJUSTL('k_Integr           = ')),k_int
        WRITE(23,"(A50,ES10.2)")TRIM(ADJUSTL('Time step      = ')),dt
        WRITE(23,"(A50,I8)")TRIM(ADJUSTL('Total time steps   = ')),t_step_total
        WRITE(23,"(A50,F4.2)")TRIM(ADJUSTL('Total time       = ')),time_total
        WRITE(23,"(A50,I2)")TRIM(ADJUSTL('PURGING STATUS     = ')),purging_status
        WRITE(23,"(A50,F12.6)")TRIM(ADJUSTL('Purging time   = ')),time_purg
        WRITE(23,"(A50,I6)")TRIM(ADJUSTL('k_Purging          = ')),k_P
        WRITE(23,"(A50,I2)")TRIM(ADJUSTL('VISCOSITY STATUS   = ')),viscosity_status
        WRITE(23,"(A50,F20.16)")TRIM(ADJUSTL('Viscosity      = ')),viscosity
        WRITE(23,"(A50,I4)")TRIM(ADJUSTL('No of saves        = ')),no_of_saves
        WRITE(23,"(A50,F20.16)")TRIM(ADJUSTL('Initial energy = ')),norm_k 
    CLOSE(23)
    END 
